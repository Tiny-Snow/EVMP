...
def run():
    ...
    for i in range(1, cfg.cross_time + 1):
        ...
        logs = output(y_train, pred_train, y_valid, pred_valid, y_test, pred_test, file_name = 'Support Vector Machine', i = i)
        for s in logs:
            log(s)

        joblib.dump(model, cfg.save_root + '/SVM_{}.pkl'.format(i))
