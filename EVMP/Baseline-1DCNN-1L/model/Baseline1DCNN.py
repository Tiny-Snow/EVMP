# -*- coding:UTF-8 -*-
# Author:   Raffica
# Project:  EVMP BASELINES -- 1D CNN (1 layer)
# Time:     2022.7.9
...
def run(train_loader, val_loader, test_loader, predict_loader = None):
    ...
    model = Baseline1DCNN(device = device, seq_len = cfg.seq_len, input_size = 5, hidden_size = [1024]).to(device)
    # signal
    ...
    result = output(y_train, pred_train, y_valid, pred_valid, y_test, pred_test, file_name = '1DCNN (1 layer)')
