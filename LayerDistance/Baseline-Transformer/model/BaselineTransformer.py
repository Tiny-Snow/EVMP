...
class BaselineTransformer(nn.Module):
    ...
    def _layer_distance(self, x):
        ...
        base = self.base_pos(base)
        base = torch.cat([base, torch.zeros(list(base.size()[: - 1]) + [self.base_embed_size - base.size(-1)]).to(self.device)], dim = -1)
        embeddings.append(base.flatten())
        # base_embedding = self.base_embed(base)
        for el in self.base_embed.layers:
            base = el(base)
            embeddings.append(base.flatten())
        base_embedding = base.view([base.size(0), -1])
...
def output(y_train, pred_train, y_test, pred_test, file_name = 'Transformer'):
