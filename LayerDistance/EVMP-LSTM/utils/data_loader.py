...
def load_data():
    ...
    with open(cfg.base_promoter_path, 'r') as f:
        ...
        for i, [ID, mopromoter, promoter, act] in enumerate(reader):
            if mopromoter in wild_promoter:
                base_promoter.append({'Moindex': wild_promoter[mopromoter]['Moindex'], 'Mopromoter': mopromoter, 'promoter': promoter, 'act': math.log10(float(act))})
            else:
                base_promoter.append({'Moindex': float('inf'), 'Mopromoter': mopromoter, 'promoter': promoter, 'act': math.log10(float(act))})
    base_bases, base_vars, base_acts = [], [], []
    for p in base_promoter:
        var_encoding = var_encode(p['Moindex'], p['Mopromoter'], p['promoter'])
        if not var_encoding is None:
            base_bases.append(onehot_encode(p['Mopromoter']))
            base_vars.append(var_encoding)
            base_acts.append(p['act'])
    base_set = VarPromoterDataset(base_bases, base_vars, base_acts)
    base_loader = DataLoader(base_set, batch_size = 1, shuffle = False, drop_last = True)
