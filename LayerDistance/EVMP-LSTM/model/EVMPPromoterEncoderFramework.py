...
class EVMPPromoterEncoderFramework(nn.Module):
    ...
    def _layer_distance(self, x):
        ...
        o, (hn, cn) = self.varLSTM(var)
        o = o[0]
        for s in range(o.size(0)):
            embeddings.append(o[s].flatten())
        var_outs = []
        for layer in range(self.var_layers):
            self.fcs[layer].cuda()
            var_out = self.fcs[layer](cn[layer])
...
def run(lr, train_loader, val_loader, test_loader, predict_loader = None, base_loader = None): 
    '''
